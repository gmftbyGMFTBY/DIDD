...
class ResultProcessor:
    ...
    def analyse_result(self, task, dataset_with_prompt, result_by_id):
        ...
        precision_score_by_type = defaultdict(float)
        recall_score_by_type = defaultdict(float)
        score_by_dataset = defaultdict(float)
        precision_score_by_dataset = defaultdict(float)
        recall_score_by_dataset = defaultdict(float)
        total_score = 0.0
        for sample in dataset_with_prompt:
            q_id = sample["id"]
            dataset_name = sample["question_source"]
        ...
        if task in ["generation", "correction"]:
            ...
            for type in check_by_type:
                ...
                accuracy_score_by_type[type] = 100 * accuracy_score(y_true=golden_labels, y_pred=pred_labels)
                recall_score_by_type[type] = 100 * recall_score(y_true=golden_labels, y_pred=pred_labels)
            for dataset in check_by_dataset:
                ...
                precision_score_by_dataset[type] = 100 * accuracy_score(y_true=golden_labels, y_pred=pred_labels)
                recall_score_by_dataset[type] = 100 * recall_score(y_true=golden_labels, y_pred=pred_labels)
            ...
            total_precision_score = 100 * accuracy_score(y_true=golden_labels, y_pred=pred_labels)
            total_recall_score = 100 * recall_score(y_true=golden_labels, y_pred=pred_labels)

        return (total_score, total_precision_score, total_recall_score), (score_by_type, precision_score_by_type, recall_score_by_type), (score_by_dataset, precision_score_by_dataset, recall_score_by_dataset)

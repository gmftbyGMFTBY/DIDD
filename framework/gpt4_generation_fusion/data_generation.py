...
def generate_train_data(few_shot, domain_dis, quality_dis):
    ...
    for _ in range(args.train_query_num):
        ...
        if quality_ in few_shot[domain_]:
            ...
            reference = ''
        for index in range(args.train_num_each):
...
if __name__ == "__main__":
    prompt = open('prompts/data_generation_v2_pairwise_v2.md').read()
    ...
    if args.mode == 'test_set_construction':
        ...
        if os.path.exists(os.path.join(args.root_path, f'iter_{args.iter_num}', f'meta_evaluation')) is False:

...
./run_stat.sh $root_path $iter_num

# 4. generate train set
failure_dis_file=${root_path}/iter_${iter_num}/failure-dis.json
# generate file: {root_path}/iter_{iter_num}/train_set_gn_{train_num_each}_fsn_{few_shot_num}.json
./run_train_set_construction.sh $root_path $iter_num $train_num_each $few_shot_num $failure_dis_file $train_query_num

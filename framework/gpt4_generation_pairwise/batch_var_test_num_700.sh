...
root_path=data_baseline_easy_test_num_20250224_test_num_700
iter_num=0
test_query_num=140
gen_num=5
few_shot_num=3
...
# 2. evaluate
# generate file: {root_path}/iter_{iter_num}/model_prediction_$test_query_num_$gen_num.json
./run_evaluate.sh $root_path $iter_num $gen_num $few_shot_num $model_path $model_bsz $cuda_gpu_index $test_query_num

# 3. prediciton
# generate file: {root_path}/iter_{iter_num}/meta_evaluation_for_{model_prediction_file_name}.json
./run_meta_evaluate.sh $root_path $iter_num $gen_num $few_shot_num $model_path $test_query_num

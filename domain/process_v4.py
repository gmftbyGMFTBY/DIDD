...
if __name__ == "__main__":
    ...
    max_base_num = 3000 // 4 + 10
    overall = []
    for key in new_data:
        if key in ['creative_writing', 'summarization', 'general_communication', 'nlp_tasks']:
            continue
        data = []
        ...
        data = random.sample(data, max_base_num)
        print(f'[!] {key}:', len(data))
        overall.extend(data)
    with open(f'data/overall_weak_4.json', 'w') as f:
